...
public class AttackSpellSlot : SpellSlot
{
   ...
   private  void Start()
    ...
    public override void Cast(Player player)
    {
        ...
        StartCoroutine(Shoot(player.shootTimeHolder,player.shootPoint.position,player.stats.Damage,player.transform.forward,player.enemyMask,player.stats.ProjectilesPerCast));
        
    }
    private IEnumerator Shoot(float shootTimeHolder,Vector3 shootPoint,float damage,Vector3 direct,LayerMask enemyMask,int count)
    {
        ...
        {
            BaseProjectile pro = Instantiate(attackSpellSO.projectile, shootPoint, Quaternion.identity);
